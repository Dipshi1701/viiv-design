(function () {
  ...
  class Search {
    ...
    searchHandler = async (resultsDB, resultsWeb) => {
      ...
      const i1 = this.settings.productquery[0].findIndex((el) =>
        s.includes(el.toLowerCase())
      );
      const i2 = this.settings.productquery[1].findIndex((el) =>
        s.includes(el.toLowerCase())
      );
      ...
      if (i1 >= 0 || i2 >= 0) {
        const pf =
          i1 >= 0
            ? this.settings.productquery[2][i1]
            : this.settings.productquery[2][i2];
        resultsWeb.searchStore.$forceFilters = [
          `PRODUCT:"${pf}"`,
          ['FORMAT:"Document"', 'FORMAT:"Video"'],
        ];
        resultsDB.searchStore.$forceFilters = [
          `PRODUCT:"${pf}"`,
          ["FORMAT:_null"],
        ];
      } else {
      ...
      try {
        ...
        const toLogId = (r) =>
          typeof r === "string" ? r : r?.response?.logId || r?.logId || "";
          ...
          if (!ctx.length) {
            ...
            const itemsHtml = ctx
              .map((c) => {
                ...
                const title =
                  (c &&
                    (c.title || c.name || c.documentTitle || c.pageTitle)) ||
                  "Source";

                return `
    <div class="sidebar-card src-card" style="gap:12px;">
  ...
  `;
            ...
            srcHost.innerHTML = `<div class="ai-sources__title">Sources used for this answer (${ctx.length}):</div>${itemsHtml}`;
    ...
    async sdkSearch() {
      ...
      const resultsSel = document.getElementById("results2")
        ? "#results2"
        : null;
      const resultsAcrossSel = document.getElementById("results")
        ? "#results"
        : null;
      const noResultsSel = document.getElementById("no-results2")
        ? "#no-results2"
        : null;
      const noResults2Sel = document.getElementById("no-results1")
        ? "#no-results1"
        : null;
      const loaderSel = document.getElementById("loader") ? "#loader" : null;
      const tabsWebSel = document.getElementById("tabs-web")
        ? "#tabs-web"
        : null;
      const tabsDbSel = document.getElementById("tabs-db") ? "#tabs-db" : null;
      const paginationWebSel = document.getElementById("pagination-web")
        ? "#pagination-web"
        : null;
      const paginationDbSel = document.getElementById("pagination-db")
        ? "#pagination-db"
        : null;

      if (!resultsSel || !resultsAcrossSel) {
        console.error(
          "Results containers missing (#results2 and/or #results)."
        );
        return;
      }
      ...
      const results = sdk.component(
        ...
        resultsSel,
        this.resultOptions({
          template: this.templates.templateSite,
          attribute: "PRODUCT",
        })
      );
      ...
      const resultsAcross = sdk.component(
        ...
        resultsAcrossSel,
        this.resultOptions({
          template: this.templates.templateDB,
          attribute: "PRODUCT",
        })
      );
      ...
      let tabsWeb = null;
      let tabsDb = null;
      let paginationWeb = null;
      let paginationDb = null;

      if (tabsWebSel) {
        tabsWeb = sdk.component("refinement-tabs", tabsWebSel, {
          attributeName: "CONTENT_TYPE",
          showFacetCount: false,
         
          tabs: [
            { facetValue: "_all" },
            { facetValue: "full-letter" },
            { facetValue: "infographic" },
            { facetValue: "video" },
            { facetValue: "congress-presentation" },
          ],
        });
        results.linkTo(tabsWeb);
      }

      if (tabsDbSel) {
        tabsDb = sdk.component("refinement-tabs", tabsDbSel, {
          attributeName: "CONTENT_TYPE",
          showFacetCount: false,
          tabs: [
            { facetValue: "_all" },
        ...
        resultsAcross.linkTo(tabsDb);
      }

      if (paginationWebSel) {
        paginationWeb = sdk.component("pagination", paginationWebSel, {

          padding: 2,
        });
        results.linkTo(paginationWeb);
      }

      if (paginationDbSel) {
        paginationDb = sdk.component("pagination", paginationDbSel, {
          padding: 2,
        });
        resultsAcross.linkTo(paginationDb);
      }
      ...
      const autocompleterSel = document.getElementById("autocompleter")
        ? "#autocompleter"
        : null;
      
      if (autocompleterSel && this.inputValue) {
        this.autocompleter = sdk.component("autocompleter", autocompleterSel, {
          behaveAsPopover: false,
          maxResults: 8,
        ...
        results.linkTo(this.autocompleter);
        this.autocompleter.setInputElement(this.inputValue);
        this.inputValue.addEventListener("keypress", (event) => {
          if (this.autocompleter.suggestion && event.keyCode === 13) {
            this.autocompleter.trackClick(this.autocompleter.suggestion);
          }
        });
        ...
        this.inputValue.addEventListener("keyup", (e) => {
          if (e.keyCode === 38 || e.keyCode === 40) {
            ...
            [].forEach.call(items, function (el, i) {
              if (i === this.autocompleter.cursor) {
      ...
      this.searchButton.addEventListener("click", () => {
        if (this.autocompleter) {
          this.autocompleter.blur();
        }
        this.searchHandler(results, resultsAcross);
      });
      ...
      results.searchStore.on("search", (res) => {
        try {
          ...
          const list = items.map(
            (it) =>
              `${it.id}-${(it.highlightedTitle || it.title || "").toString()}`
          );
          setTimeout(
            () => sdk.client.trackEvent("Results", { value: list }),
            50
          );
        } catch (e) {
